...
void DataDistributionInput::copy_tensor_vec(const std::vector<core23::Tensor> &dp_keys,
  ...
  for (size_t i = 0; i < num_lookup_; ++i) {
    ...
    h_ptrs_.template data<void *>()[i] = dp_keys[dr_lookup_id].data();
    // ATTENTION:
    ...
    if (dp_bucket_range.size() == dp_keys.size()) {
      h_ptrs_.template data<void *>()[num_lookup_ + i] = dp_bucket_range[dr_lookup_id].data();
    } else {
      HCTR_CHECK(num_lookup_ == dp_bucket_range.size());
      h_ptrs_.template data<void *>()[num_lookup_ + i] = dp_bucket_range[i].data();
    }
  }
...
void DynamicWgradInitiazlier::init_indices_and_data(Wgrad &wgrad, size_t h_num_key,
  ...
  HCTR_LIB_THROW(cudaMemcpy(&h_unique_key_num,
                            key_flag_buffer.template data<uint32_t>() + h_num_key - 1,
